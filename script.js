...
function setupEventListeners() {
    ...
    // GitHub水印点击事件
    const githubWatermark = document.getElementById('githubWatermark');
    if (githubWatermark) {
        githubWatermark.addEventListener('click', () => {
            // 添加点击反馈效果
            githubWatermark.style.transform = 'scale(0.95)';
            setTimeout(() => {
                githubWatermark.style.transform = '';
            }, 150);
        ...
        // 添加悬停音效模拟（视觉反馈）
        githubWatermark.addEventListener('mouseenter', () => {
            githubWatermark.style.filter = 'brightness(1.1)';
        });
        
        githubWatermark.addEventListener('mouseleave', () => {
            githubWatermark.style.filter = 'brightness(1)';
        });
    }
}
